...
XMFLOAT2 VectorBoard::getClosestField(XMFLOAT2 position, XMFLOAT2 velocity, vector<MagneticObject*> magneticObjects) {
	...
	// add the magnetic contribution to it
	XMFLOAT2 resultant = board[x][y]->getFieldLine();
	XMFLOAT2 magnetic = getMagneticSum(magneticObjects, position, velocity);

	resultant.x += magnetic.x;
	resultant.y += magnetic.y;

	return resultant;
}

XMFLOAT2 VectorBoard::getMagneticSum(vector<MagneticObject*> magneticObjects, XMFLOAT2 position, XMFLOAT2 velocity) {
	XMFLOAT2 sum(0, 0);

	for (MagneticObject* object : magneticObjects) {
		XMFLOAT2 single = object->calculateField(position, velocity);
		sum.x += single.x;
		sum.y += single.y;
	}
